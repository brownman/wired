 ...
 *
 * File: ImageLoader.c
 * Desc: OpenGL C Port of Delphi BMP and TGA Image Loading utility (From SGI)
 *		 This came undocumented. Guaranteed to work.
 *****************************************************************************/
...
int glBmpLoadBMP(glBmpImage* img, char *filename)
{
	...
	if (!(bmpFile = fopen(filename, "rb"))) return 0;
...
int glBmpMemLoadBMP(glBmpImage* img, char *filePtr)
{
	...
	if (fHeader.bfType != BM) return 0;

	memcpy(&iHeader, filePtr, sizeof(iHeader));
	filePtr += sizeof(iHeader);

	if (iHeader.biSize == sizeof(BITMAPCOREHEADER)) return 0;
...
int glBmpMemLoadTGA(glBmpImage* img, char *filePtr)
{
	...
	if ((img->colorDepth != 24) && (img->colorDepth != 32)) return 0;
